...
// ── NEXT READ — AI-picked recommendation from unread pile ──────────────────
;(function () {
  const NR_SKIP_DAYS = 14;

  function nrSkippedKey() { return 'tsundoku_nr_skipped_' + currentUser.id; }
  function nrCacheKey() {
    const today = new Date().toISOString().slice(0, 10);
    return 'tsundoku_next_read_' + currentUser.id + '_' + today;
  }

  function nrGetSkipped() {
    try { return JSON.parse(localStorage.getItem(nrSkippedKey()) || '[]'); } catch { return []; }
  }
  function nrAddSkipped(bookId) {
    const list = nrGetSkipped().filter(s => s.book_id !== bookId);
    list.push({ book_id: bookId, skipped_at: Date.now() });
    try { localStorage.setItem(nrSkippedKey(), JSON.stringify(list)); } catch {}
  }
  function nrActiveSkipIds() {
    const cutoff = Date.now() - NR_SKIP_DAYS * 86400000;
    return new Set(nrGetSkipped().filter(s => s.skipped_at > cutoff).map(s => s.book_id));
  }

  function nrGetUnread() {
    const skipped = nrActiveSkipIds();
    return books.filter(b => b.status === 'unread' && !isHiddenFromShelf(b) && !skipped.has(b.id));
  }

  function nrScoreCandidates() {
    const readBooks = books.filter(b => b.status === 'read');
    const genreFreq = {}, authorFreq = {}, themeFreq = {};
    readBooks.forEach(b => {
      getBookGenres(b).forEach(g => genreFreq[g] = (genreFreq[g] || 0) + 1);
      if (b.author) authorFreq[b.author] = (authorFreq[b.author] || 0) + 1;
      (Array.isArray(b.themes) ? b.themes : []).forEach(t => themeFreq[t] = (themeFreq[t] || 0) + 1);
    });
    const pool = nrGetUnread();
    const scored = pool.map(b => {
      let score = 0;
      getBookGenres(b).forEach(g => { if (genreFreq[g]) score += 4 * genreFreq[g]; });
      if (b.author && authorFreq[b.author]) score += 3 * authorFreq[b.author];
      (Array.isArray(b.themes) ? b.themes : []).forEach(t => { if (themeFreq[t]) score += 2 * themeFreq[t]; });
      return { book: b, score };
    });
  ...
  function nrTbrAgeReason(book) {
    if (!book.created_at) return null;
    const days = Math.floor((Date.now() - new Date(book.created_at)) / 86400000);
    if (days < 3) return null;
    const label = days < 30 ? `${days} days` :
      new Date(book.created_at).toLocaleDateString('en-US', { month: 'long' }).toLowerCase();
...
Candidates:
${candidates.map(b => `id:${b.id} | "${b.title}" by ${b.author || 'unknown'} | genres: ${getBookGenres(b).join(', ') || 'none'}`).join('\n')}`;
  ...
  async function nrGenerateRecommendation() {
    ...
    const ageReason = nrTbrAgeReason(book);
  ...
  async function nrGetOrGenerate(force) {
    if (!force) {
      try {
        const cached = JSON.parse(localStorage.getItem(nrCacheKey()) || 'null');
  ...
  const NR_ICONS = {
    author: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>',
    theme: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v3M12 18v3M3 12h3M18 12h3"/></svg>',
    genre: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.59 13.41 13.42 20.58a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/></svg>',
    mood: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>',
    pacing: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 12H4M4 12l6-6M4 12l6 6"/></svg>',
    tone: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/></svg>',
    tbr_age: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 16 14"/></svg>',
    page_count: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>'
  };

  let nrCurrentRec = null;
  let nrExpanded = false;

  function nrCoverHtml(book) {
    return book.cover_url
      ? `<img src="${escapeAttr(book.cover_url)}" style="width:100%;height:100%;object-fit:cover" draggable="false"/>`
    ...
    if (!nrExpanded) {
      card.innerHTML = `
        ...
        <div class="nr-collapsed" id="nrCollapsedRow">
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" stroke-width="2.2"><polyline points="9 18 15 12 9 6"/></svg>
        </div>`;
      document.getElementById('nrCollapsedRow').addEventListener('click', () => { nrExpanded = true; nrRenderCard(); });
      return;
    }

    const reasonRows = (rec.supporting_reasons || []).map(r => `
      <div class="nr-reason-row">
        <span class="nr-reason-icon">${NR_ICONS[r.type] || NR_ICONS.genre}</span>
      ...
      <div class="nr-expanded">
        ...
        <div class="nr-book-row">
          ...
          <div class="nr-book-info">
            ...
            <div class="nr-headline">${escapeHtml(rec.headline_reason.text)}</div>
          </div>
        </div>
        <div class="nr-reasons">${reasonRows}</div>
        <div class="nr-actions">
          <button class="nr-start-btn" id="nrStartBtn">start reading</button>
          <button class="nr-skip-btn" id="nrSkipBtn">not this one</button>
        </div>
      </div>`;

    document.getElementById('nrStartBtn').addEventListener('click', async () => {
      const b = books.find(bk => String(bk.id) === String(book.id));
      if (!b) return;
      b.status = 'reading';
      renderGrid();
      await dbUpdate(book.id, { status: 'reading' });
      try { localStorage.removeItem(nrCacheKey()); } catch {}
      nrExpanded = false;
      showToast(`moved "${b.title}" to reading ✓`);
      nrRenderCard();
    });
